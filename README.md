...
## 📄 ライセンスおよびリンクについて (License & Linking)
本リポジトリ内のコードおよびファイルの著作権は作成者に帰属します（All Rights Reserved）。

* **無断転載・再配布の禁止:** 本リポジトリ内のスクリプト（`.user.js`）やデッキファイル（`.apkg`）を、自身のサイトや他のリポジトリで二次配布・再アップロードすることはご遠慮ください。
* **リンクフリー:** 本リポジトリへのリンクは完全フリーです！ブログ、SNS、QiitaやZennなどの技術記事での紹介は大歓迎ですので、ツールを紹介していただける際は、ぜひこちらのGitHubリポジトリのURLをシェアしてください。